...
const TaskInfo = (props) => {
  ...
  const onSearch = (value) => {
    if (value) {
      ...
      filteredList.map((item) => {
        emitter.emit("map-move-focus", [item.lon, item.lat], 1000);
      });
  ...
  return (
    <div style={{ height: "100%" }}>
      ...
      <Modal
        mask={false}
        className="person-info-modal"
        visible={personInfoVisible}
        onCancel={() => setPersonInfoVisible(false)}
        footer={
          <div
            style={{ display: "flex", flexDirection: "row" }}
            className={"person-info-line"}
          >
            <DatePicker.RangePicker
              showTime
              defaultValue={[selectedBeginTime, selectedEndTime]}
              onChange={(dates) => {
