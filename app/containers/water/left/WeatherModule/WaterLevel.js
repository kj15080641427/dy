  ...
  selectInit() {
    ...
    getBasicsAll({
      ...
      for (let i = 0; i < list.length; i++) {
        if (list[i].region === "370502" && list[i].indtype !== 11) {
          dyarr.push(list[i]);
        }
        if (list[i].region === "370523") {
          grarr.push(list[i]);
        }
        if (list[i].region === "370522") {
          ljarr.push(list[i]);
        }
        if (list[i].region === "370503") {
          hkarr.push(list[i]);
        }
        if (list[i].region === "370521") {
          klarr.push(list[i]);
        } else if (list[i].indtype === 11) {
          teharr.push(list[i]);
        }
      }
      let data = [
        { regionName: "全部", list: list },
