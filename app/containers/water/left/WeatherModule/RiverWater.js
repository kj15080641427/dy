...
class RiverWater extends React.PureComponent {
  ...
  showModal = (obj) => {
    ...
    let starttm = moment(new Date().getTime() - 24 * 60 * 60 * 1000).format(
      "YY-MM-DD HH:mm:ss"
    );
  ...
  onExpandOpen = (expanded, record) => {
    ...
    getWfsRiverByName({
      ...
      for (
        let i = 0;
        i < result.data[0].siteBaseDo?.siteWaterLevels.length;
        i++
      ) {
        const item = result.data[0].siteBaseDo?.siteWaterLevels[i];
        data.push({
          stcd: item.stcd,
          SpliceSiteName: item.name + "(" + item.dataSourceDesc + ")",
          z: item.z || "-",
    ...
    const expandedRowRendertype = (record, index, indent, expanded) => {
      ...
      const swcolumns = [
        ...
        },
        {
          ...
          className: "column-money",
          render: (warning) =>
            warning != "99" ? (warning * 1).toFixed(2) : "-",
        ...
        },
        {
          ...
          sorter: (a, b) =>
            new Date(a.ztm).getTime() - new Date(b.ztm).getTime(),
        },
      ];
      console.log(this.state.zdDataSource, "============");
      ...
      return React.cloneElement(
        <Table
          ...
          pagination={{
            showTotal: () =>
              `共${this.state.zdDataSource[record.name].length}条`,
          }}
        />
      );
    };
