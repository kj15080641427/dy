...
import "../style.scss";
import { Table, Space, Popover, Button, Input, Drawer, message } from "antd";
...
class Precipitation extends React.PureComponent {
  ...
  render() {
    ...
    const expandedRowRendertype = (record) => {
      console.log(record);
      const qycolumns = [
        {
          title: "站名",
          ...
          <div>
            ...
            >
              {this.state.videoobj !== null ? (
                <VideoComponent
                  videoControl={this.state.videoobj}
  ...
  selectInit() {
    ...
    getAllVideo({
      ...
      for (let i = 0; i < result.data.length; i++) {
        if (result.data[i].regionName === "东营区") {
          if (result.data[i].dataSource * 1 === 10) {
            ...
            result.data[i].dataSource * 1 === 3 ||
            result.data[i].dataSource * 1 === 9
          ) {
      ...
      let dyarr = [
        { name: "河道", list: dycatyarr },
        { name: "积水点", list: dyfloodarr },
        { name: "天鹅湖蓄滞洪区", list: dyteharr },
      ];
