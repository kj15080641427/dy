...
class Rain extends React.PureComponent {
    ...
    render() {
        ...
        let element = []
        for (let i = 0; i < floodcount.length; i++) {
            element.push(
                <>
                    < a > {floodcount[i].dataSourceDesc}共计：{floodcount[i].number}个</a >&nbsp;
                </>
            )
            ...
            <div>
                <Row>
                    <div className="div-left-echarts">
                        <div className="echarts-isOnline" id="floodisOnline"></div>
                        <div className="echarts-count" id="floodcount"></div>
                    </div>
    ...
    selectRainSource = () => {
        getCountStation({
            ...
            console.log(result)
            this.setState({
                count: result.data[2].number,
                floodcount: result.data[2].list
            })
            let nameArr = [];
            let data = [];
            for (let i = 0; i < result.data[2].list.length; i++) {
                nameArr.push(result.data[2].list[i].dataSourceDesc === null ? "暂无数据" : result.data[2].list[i].dataSourceDesc)
                data.push({
                    'name': result.data[2].list[i].dataSourceDesc === null ? "暂无数据" : result.data[2].list[i].dataSourceDesc,
                    'value': result.data[2].list[i].number
                })
            }
            var myChart = echarts.init(document.getElementById("floodcount"));
            myChart.setOption({
                title: {
                    text: '易涝站点来源统计图',
                    // subtext: '纯属虚构',
            ...
            .then((result) => {
                ...
                for (let i = 0; i < result.data.length; i++) {
                    if (result.data[i].ztm === null) {
                        nocount++;
                    } else if (startdata - new Date(result.data[i].ztm).getTime() >= 1000 * 60 * 60 * 24 * 3) {
                        thday++;
                    } else {
                        okcount++;
                    }
                ...
                console.log(nocount)
                console.log(thday)
                var myChartcount = echarts.init(document.getElementById("floodisOnline"));
