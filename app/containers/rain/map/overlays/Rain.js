...
class Rain extends Base {
  ...
  render() {
    ...
    const {minuteAvg, hourAvg, dayAvg} = model;
    //let drpLevel = (model.drp !== null && model.drp !== undefined) ? model.drp + 'mm' : '--';
    let drpLevel = (minuteAvg === null || minuteAvg === undefined) ? '--' : (minuteAvg * 1).toFixed(1) + 'mm';
    let hourRain = hourAvg=== null ||minuteAvg === undefined  ? '--' : ((hourAvg * 1).toFixed(1) + 'mm');
    let dayRain = (dayAvg === null || dayAvg === undefined) ? '--' : ((dayAvg * 1).toFixed(1) + 'mm');
