  ...
  render() {
    ...
    return (
      <div className="m-alm-table">
        ...
        <div className="m-alm-div">
          <Row className="m-alm-row">
            <Col span={4}>
              <img className="m-alm-warningImg" src={warningImg}></img>
            </Col>
            <Col span={20}>
              <Row>
                <Col span={12}>
                  超警戒水位
                  <span className="m-alm-row-warning">
                    {this.state.wacount}
                  </span>
                </Col>
          ...
          <Table
            className="m-alm-div-table"
            size="small"
            loading={loading}
            columns={columns}
            dataSource={this.state.wardataSource}
            scroll={{ y: 320 }}
            rowKey={(row) => row.stcd}
          ...
          >
            <Row>
              <Col span={12}>
                <Card title="水位走势" bordered={false}>
                  <div id="mainbywat" style={{ width: 600, height: 500 }}></div>
                </Card>
              </Col>
              <Col span={12}>
                <Card title="水位数据" bordered={false}>
                  <Table
                    size="small"
  ...
  selectInit() {
    ...
    getWaterWarning({}).then((result) => {
      ...
      for (let i = 0; i < result.data.length; i++) {
        if (
          moment(result.data[i].alarmtime).format("YYYY-MM-DD") ===
